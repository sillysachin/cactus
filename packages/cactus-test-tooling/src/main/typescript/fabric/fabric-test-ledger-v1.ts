...
export class FabricV1TestLedger implements ITestLedger {
  ...
  constructor(
    ...
    this.opsApiHttpPort =
      options.opsApiHttpPort ||
      FABRIC_TEST_LEDGER_DEFAULT_OPTIONS.opsApiHttpPort;
  ...
  public destroy(): Promise<any> {
    ...
    if (this.container) {
      ...
      return Promise.reject(
        new Error(`${fnTag} Containernot found, nothing to destroy.`)
      );
    }
  }
  ...
  protected async getContainerInfo(): Promise<ContainerInfo> {
    ...
    if (aContainerInfo) {
      ...
      throw new Error(
        `FabricV1TestLedger#getContainerInfo() no image "${image}"`
      );
    }
  }
